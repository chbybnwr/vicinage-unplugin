    ...
    handler: (code: string, id: string) => {
      ...
      traverse(ast, {
        ...
        CallExpression: (path) => {
          ...
          const macro = getMacro(path.scope.getBinding(callee.name))
...
function getMacro(binding: Binding | undefined): string | null {
  ...
  if (!parentNode.source.value.includes(pluginName)) {
    return null
  }
